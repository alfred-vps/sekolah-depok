#!/bin/bash
# Deploy riset-sd to Cloudflare Pages
...
for name in SUPABASE_URL SUPABASE_SERVICE_KEY SUPABASE_ANON_KEY PASS_APPS_RISETSD; do
  value="${!name}"
  if [ -n "$value" ]; then
    echo "$value" | wrangler pages secret put "$name" --project-name riset-sd 2>/dev/null && echo "  ✓ $name" || echo "  ✗ $name"
  else
...
wrangler pages deploy "$OUTPUT_DIR" --project-name riset-sd --branch main

echo "✅ Deploy complete!"
echo "🌐 https://riset-sd.pages.dev"
